%% Run Tests — Option 4 Compression + Simulink + Codegen Pipeline
% =========================================================================
% Runs the full test suite for Option 4 pipeline output.
% Must be run from option4_compressed/ directory.
%
% Usage:
...
function run_tests(tag)
    ...
    fprintf('==========================================================\n');
    fprintf('  Option 4 Pipeline Tests\n');
    fprintf('  %s\n', char(datetime('now')));
    if ~isempty(tag)
    ...
    if isempty(tag)
        suite = testsuite('TestPipeline_opt4');
    else
        suite = testsuite('TestPipeline_opt4', 'Tag', tag);
    end
